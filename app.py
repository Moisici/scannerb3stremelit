...
def analyze_ticker(ticker, df):
    ...
    return {
        ...
        "min52": round(float(min_52), 2),
        "max52": round(float(max_52), 2),
...
st.info("Sincronização em Lote (Batch) para o Firebase.")
...
if st.button("⚡ INICIAR SINCRONIZAÇÃO"):
    ...
    if db:
        ...
        for i, ticker in enumerate(ACOES_B3):
            ...
            time.sleep(0.02) # Levemente mais rápido sem o render da tabela
        
        if count > 0:
            status_text.text("🚀 Enviando para o Firebase...")
            try:
                batch.commit()
                # Removemos a tabela e deixamos apenas o sucesso
                st.success(f"✅ Concluído! {count} ativos sincronizados com sucesso.")
            except Exception as e:
